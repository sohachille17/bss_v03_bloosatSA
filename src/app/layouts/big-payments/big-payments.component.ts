...
@Component({
  selector: 'app-big-payments',
  templateUrl: './big-payments.component.html',
  styleUrls: ['./big-payments.component.css']
})
export class BigPaymentsComponent {
  ...
  constructor(
    ...
    private customerService: BigCustomerServiceService,
  ...
  onGetBillID(){
    ...
    this.customerService.getBigCustomerBill(this.globalCustomerId).subscribe(uniqueBill => {
      // console.log("====>>>>",uniqueBill)
      this.customerUniqueBills = uniqueBill
      console.log("UNIQUE",this.customerUniqueBills)
...
fetch(`${this.BLOO__CONNECT}big-payments`,  {
  method: 'POST',
  headers: myHeaders,
  body: formdata,
  redirect: 'follow',
  ...
  onGetallPaymentsbyID(){
    ...
    this.customerService.getPaymentsByID(this.globalCustomerId).subscribe(customPayment => {
  ...
  onLoadPayment(id: any){
    this.router.navigateByUrl(`/big-payment-receipte?id=${id}`);
  }
  ...
  selectPayment(): void {
    ...
    this.customerService.getBigInvoice(billID).subscribe((uniquebillsID) => {
      console.log(" ...VALUE HERE ==>",uniquebillsID)
      let billData: any
      billData = uniquebillsID

      this.paymentForm.get('siteName')?.setValue(billData.customerName)
      this.paymentForm.get('amount')?.setValue(billData.total);
      this.paymentForm.get('id')?.setValue(billData.billNumber)
    })
